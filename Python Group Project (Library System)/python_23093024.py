...
def add_book(isbn_list, author_list, title_list, publisher_list, genre_list, year_published_list,
    ...
    for x in range(add_num):
        ...
        # While loop to allow user to have multiple attempts when they fail to enter the input
        while True:
            add_author = input("Author of the book: ").title() # capitalizing first letter of each word
            ...
            if add_author.istitle() == False:  # making sure input is not numbers or special symbols
...
def display(isbn_list, author_list, title_list, publisher_list, genre_list, year_published_list,
    ...
    # Iterate over each book and print its details in a grouped format
    for i in range(len(isbn_list)):
        print(f"{i + 1}. ISBN: {isbn_list[i]}")
        print(f"   Author: {author_list[i]}")
        print(f"   Title: {title_list[i]}")
        print(f"   Publisher: {publisher_list[i]}")
        print(f"   Genre: {genre_list[i]}")
        print(f"   Year Published: {year_published_list[i]}")
        print(f"   Date Purchased: {date_purchased_list[i]}")
        print(f"   Status: {book_status_list[i]}")
        print()
